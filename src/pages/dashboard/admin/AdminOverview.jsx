    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard
          title="Total Clubs"
          value={
            (stats?.totalClubs?.pending || 0) + (stats?.totalClubs?.approved || 0) + (stats?.totalClubs?.rejected || 0)
          }
          icon={HiOutlineBuildingOffice2}
          color="secondary"
        />
        <StatCard title="Total Memberships" value={stats?.totalMemberships || 0} icon={FiUsers} color="accent" />
        <StatCard
          title="Total Revenue"
      ...
      <div className="grid lg:grid-cols-3 gap-4 mb-8">
        <div className="card bg-base-100 shadow-sm">
          <div className="card-body">
            ...
            <p className="text-3xl font-bold">{stats?.totalClubs?.pending || 0}</p>
          </div>
        </div>
        <div className="card bg-base-100 shadow-sm">
          <div className="card-body">
            <h3 className="font-semibold text-success">Approved Clubs</h3>
            <p className="text-3xl font-bold">{stats?.totalClubs?.approved || 0}</p>
          </div>
        </div>
        <div className="card bg-base-100 shadow-sm">
          <div className="card-body">
            <h3 className="font-semibold text-error">Rejected Clubs</h3>
            <p className="text-3xl font-bold">{stats?.totalClubs?.rejected || 0}</p>
          </div>
        </div>
      </div>
