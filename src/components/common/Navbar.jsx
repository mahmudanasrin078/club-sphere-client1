    ...
    <div className="">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center gap-4">
            {user ? (
              <div className="dropdown dropdown-end">
                ...
                <ul tabIndex={0} className="menu dropdown-content z-50 p-2 shadow-lg bg-base-100 rounded-box w-52 mt-4">
                  <li className="menu-title">
                    <span className="text-sm font-semibold">{user.displayName}</span>
                    <span className="text-xs text-base-content/60 capitalize">{dbUser?.role || "Member"}</span>
                  </li>
                  <li>
          ...
          {/* Mobile Menu Button */}
          <button className="md:hidden btn btn-ghost btn-square" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
            ...
            <div className="flex flex-col gap-2">
              {navLinks.map((link) => (
                <NavLink
                  ...
                  className={({ isActive }) => `px-4 py-2 rounded-lg ${isActive ? "bg-primary/10 text-primary" : ""}`}
                >
                  {link.label}
                </NavLink>
              ))}
